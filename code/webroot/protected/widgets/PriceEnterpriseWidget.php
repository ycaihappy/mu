...
class PriceEnterpriseWidget extends CWidget
{
    ...
    public function run()
    {
		...
		$entCriteria->with=array('user'=>array('select'=>'user_telephone,user_mobile_no,user_first_name,user_name'));
		...
		if($advEnt)
		{
			...
			{
				$ent->ent_id=$ent->ent_website?$ent->ent_website:$this->getController()->createUrl('/storeFront/default/index',array('username'=>$ent->user->user_name));
			}
		}
        $this->render('price_enterprise',array('advEnt'=>$advEnt));
    }
}
